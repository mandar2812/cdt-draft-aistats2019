...
input pattern $x(t_i)$, returns a time delay $\Delta$ corresponding the the time step $i + \Delta$ when
...
within which the model searches for probably temporal causal links.
...
causal window $[t+\ell, t+\ell+h]$, and simultaneosly supplies a probability 
$\hat{p}(t+i)$ which represents the likelihood of a causal 
link between $x(t)$ and $y(t+i)$.
...
& \widetilde{p}^{(m)} = softmax \left( \frac{1}{T} (y^{(m)} - \hat{y}^{(m)})^{2} \right)
\end{align}
