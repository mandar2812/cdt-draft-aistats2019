...
This is paticularly challangeing given the nature of the CDT problem, although causal time lag
...
This barrier can be surpassed in turn by careful construction of synthetic data sets
which incorporate dynamic time lag relationships between causes and effects.

Synthetic data sets allow us to evaluate the accuracy of both, the output and time
lag predictions of CDT models. To this end, we the authors propose four benchmark 
tests which we hope can become cannonical examples in the area of CDT modelling.
...
\begin{enumerate}
\item Problem I Constant Lag: \newline $\Delta(t) = k$


\item Problem II Constant Velocity $||x(t)||^2$; Fixed Distance $d$: \newline $\Delta(t) = d/(\alpha ||x(t)||^2)$


\item Problem III Constant Acceleration $a$; Fixed Distance $d$: \newline $\Delta(t) = (\sqrt{\alpha^2||x(t)||^4 - 2ad} - \alpha||x(t)||^2)/a$
\end{enumerate}
